...
function daysBetween(start, end) {
  ...
  if (!parsedStart || !parsedEnd) {
    throw new TypeError(
      `Invalid date input to daysBetween: start=${start}, end=${end}`,
    );
  }
...
function normalizeTask(task) {
  const start =
    parseDate(task.start_date) || parseDate(task.created_at) || new Date();
  ...
  return (
    <div
      className={clsx(
        ...
        conflict
          ? "border-[#ba1a1a] ring-2 ring-[#ba1a1a]/15"
          : "border-white/70",
      )}
      style={{
        left: position.x,
...
export default function TimelineView() {
  ...
  const loadTimeline = useCallback(async () => {
    ...
    try {
      ...
      if (selectedFilters.workstream)
        params.set("workstream", selectedFilters.workstream);
      if (selectedFilters.tag) params.set("tag", selectedFilters.tag);
      if (selectedFilters.assignee)
        params.set("assignee", selectedFilters.assignee);

      const [projectRes, timelineRes] = await Promise.all([
        api.get(`/projects/${projectId}`),
        api.get(
          `/projects/${projectId}/timeline${params.toString() ? `?${params.toString()}` : ""}`,
        ),
      ]);
      ...
      setError(
        err.response?.data?.error || "Failed to load the project timeline.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const conflictEdges = useMemo(() => {
    return new Set(
      conflicts
        .filter(
          (conflict) =>
            conflict.type === "dependency_order" && conflict.dependency_id,
        )
  ...
  const workstreamColorMap = useMemo(() => {
    const streams = Array.from(
      new Set(tasks.map((task) => task.workstream)),
    ).sort();
    ...
    Array.from(grouped.entries())
      ...
      .forEach(([workstream, streamTasks]) => {
        output.push({
          ...
          name: workstream,
          height: GROUP_HEIGHT,
          count: streamTasks.length,
        });
        streamTasks
          .sort(
            (a, b) =>
              a.startDate - b.startDate || a.deadlineDate - b.deadlineDate,
          )
          .forEach((task) =>
            output.push({
              type: "task",
    ...
    timersRef.current[taskId] = setTimeout(async () => {
      try {
        ...
        setTasks((prev) =>
          prev.map((task) =>
            task.id === taskId
              ? normalizeTask({ ...task, ...data.task, name: data.task.title })
              : task,
          ),
        );
        await loadTimeline();
        ...
        toast.error(
          err.response?.data?.error || "Failed to update the deadline.",
        );
        await loadTimeline();
      }
    }, 500);
  ...
  if (loading) {
    return (
      <Layout
        activePath={`/projects/${projectId}/timeline`}
        projectId={projectId}
      >
        <TimelineSkeleton />
      </Layout>
      ...
      <div className="flex flex-col h-full bg-[#f3f4f5]">
        <div className="bg-white border-b border-[#e1e3e4] px-6 py-4 flex flex-col gap-4">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-xs text-[#555f6d] mb-1">
                <Link
                  ...
                  className="inline-flex items-center gap-1 hover:text-[#003fb1]"
                >
              ...
              <span
                className={clsx(
                  ...
                  conflicts.length
                    ...
                    : "bg-[#dcefe5] border-[#b8dac9] text-[#005438]",
                )}
              >
                {conflicts.length ? (
          ...
          <div className="flex items-end gap-3 flex-wrap">
            ...
            <FilterSelect
              label="Workstream"
              value={selectedFilters.workstream}
              onChange={(value) => updateFilter("workstream", value)}
            >
              <option value="">All workstreams</option>
              {filters.workstreams.map((item) => (
          ...
          <div className="flex-1 overflow-auto p-6">
              ...
              <div className="overflow-auto">
                <div style={{ width: LEFT_WIDTH + timeline.chartWidth }}>
                  <div className="sticky top-0 z-30 flex bg-white border-b border-[#e1e3e4]">
                    ...
                    <div
                      className="flex"
                      style={{ width: timeline.chartWidth }}
                    >
                        ...
                        row.type === "group" ? (
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <span
                                ...
                                style={{
                                  backgroundColor: workstreamColorMap[row.name],
                                }}
                              />
                          ...
                          <div
                            ...
                            className={clsx(
                              ...
                              conflictMap[row.task.id]?.length &&
                                "bg-[#fff7f5]",
                            )}
                            style={{ height: row.height }}
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span className="text-sm font-semibold text-[#191c1d] truncate">
                                {row.task.name}
                              </span>
                            </div>
                            <div className="text-[11px] text-[#737686] truncate">
                              {row.task.assignee_name || "Unassigned"} -{" "}
                      ...
                      <div className="absolute inset-0 z-0">
                        {rows.map((row) => (
                          <div
                            key={`${row.id}-grid`}
                            className={clsx(
                              "border-b border-[#eef0f1]",
                              row.type === "group" && "bg-[#f8fafc]",
                            )}
                            style={{ height: row.height }}
                          />
                        ))}
            ...
            {conflicts.length > 0 && (
              <div className="mt-4 bg-white border border-[#e1e3e4] rounded-lg p-4">
                <h2 className="text-sm font-bold text-[#191c1d] mb-3">
                  Scheduling conflicts
                </h2>
                <div className="grid gap-2">
                  {conflicts.map((conflict, index) => (
                    <div
                      key={`${conflict.type}-${conflict.task_id}-${index}`}
                      className="flex items-start gap-2 text-sm text-[#434654]"
                    >
                      <AlertTriangle
                        className={clsx(
