...
async function seed() {
  ...
  try {
    ...
    await pool.query("DELETE FROM users WHERE email LIKE '%@mock.collabagent.test'");
    ...
    const projectResult = await pool.query(`
      ...
      VALUES (gen_random_uuid(), 'AI-Augmented Workflow Rollout', 'Deploying the new predictive task management system.', $1)
      RETURNING id;
    `, [users[0].id]);
    ...
    for (const u of users) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3)
      `, [projectId, u.id, u.id === users[0].id ? 'owner' : 'member']);
    }

    // 5. Create Mock Tasks
    console.log('Inserting mock tasks...');
    const taskData = [
      { title: 'API Endpoint Design', status: 'done', priority: 'high', est: 6, tags: ['api','design'], assignee: users[0].id },
      { title: 'Database Schema Migration', status: 'in_progress', priority: 'critical', est: 4, tags: ['database'], assignee: users[1].id },
      { title: 'Frontend Kanban Board', status: 'blocked', priority: 'medium', est: 18, tags: ['frontend','design'], assignee: users[2].id },
      { title: 'AI Suggestion Engine', status: 'todo', priority: 'high', est: 14, tags: ['ai','api'], assignee: null },
      { title: 'Integration Testing', status: 'todo', priority: 'medium', est: 8, tags: ['testing'], assignee: null }
    ];
    ...
    for (const t of taskData) {
      const res = await pool.query(`
        ...
        RETURNING id;
      `, [projectId, t.title, t.status, t.priority, t.est, t.tags, t.assignee, users[0].id]);
