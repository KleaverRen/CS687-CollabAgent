...
function detectBlockers(tasks, edges) {
  ...
  for (const edge of edges) {
    ...
    const childBlocked = child.status === 'blocked' || child.status === 'in_progress';
    ...
    const severity =
      cascadeCount >= 3 || (daysUntilDeadline !== null && daysUntilDeadline <= 2)
        ? 'critical'
        : cascadeCount >= 1 || (daysUntilDeadline !== null && daysUntilDeadline <= 7)
        ? 'warning'
        : 'info';
    ...
    suggestions.push({
      ...
      description: `"${child.title}" cannot progress until "${parent.title}" is marked done.${
        cascadeCount > 0 ? ` This also cascades to ${cascadeCount} downstream task(s).` : ''
      ...
      confidence: Math.min(0.99, 0.6 + cascadeCount * 0.1 + (severity === 'critical' ? 0.2 : 0)),
...
function recommendSplits(tasks, edges) {
  ...
  for (const task of tasks) {
    ...
    for (const key of Object.keys(SPLIT_TEMPLATES)) {
      if (tags.some((t) => t.includes(key))) { templateKey = key; break; }
    }
...
function computeCriticalPath(tasks, edges) {
  ...
  for (const t of tasks) { childMap[t.id] = []; parentMap[t.id] = []; }
  ...
  const inDegree = Object.fromEntries(tasks.map((t) => [t.id, parentMap[t.id]?.length || 0]));
  ...
  const dist = Object.fromEntries(tasks.map((t) => [t.id, parseFloat(t.estimated_hours) || 1]));
  const prev = {};
  for (const id of order) {
    for (const child of (childMap[id] || [])) {
      const newDist = dist[id] + (parseFloat(taskMap[child]?.estimated_hours) || 1);
  ...
  const criticalPath = [];
  let cur = sink.id;
  while (cur) { criticalPath.unshift(cur); cur = prev[cur]; }
  ...
  const scored = tasks
    ...
    .map((t) => {
      const daysLeft = t.deadline
        ...
        : 999;
      const deadlineScore = daysLeft === 0 ? 10 : Math.max(0, 10 - daysLeft * 0.5);
      ...
      if (targetTask && members.length > 0) {
        ...
        if (ranked.length > 0) {
          affinitySuggestions = [{
            ...
            action: { patch_task_id: task_id, assigned_to: ranked[0]?.user_id },
            confidence: ranked[0] ? ranked[0].match_pct / 100 : 0.1,
            confidence_formula: 'tag_overlap / total_completed_tasks × 0.84 + 0.15',
          }];
        }
      }
    }
